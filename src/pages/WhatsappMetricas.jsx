...
import { getLeads, getProductGroups, getMessageLogs, getFunnelSends, getWhatsappFunnels, getDisparos } from '../lib/firestore'
import { canonicalEvento } from '../lib/constants'
import PageShell, { Panel } from '../components/PageShell'
import PageLoader from '../components/PageLoader'
import Select from '../components/Select'
import WhatsAppIcon from '../components/WhatsAppIcon'
import { Send, RefreshCw, ShoppingBag, TrendingDown, DollarSign, Search, X, ChevronLeft, ChevronRight, CheckCircle2 } from 'lucide-react'
...
/** Interpreta o valor: inteiro puro = centavos; senão lê o número formatado. */
...
export default function WhatsappMetricas() {
  ...
  const carregar = async () => {
    if (!user?.uid) return
    setLoading(true)
    const [ld, gs, mlogs, sends, wfs, disp] = await Promise.all([
      getLeads(user.uid), getProductGroups(user.uid), getMessageLogs(user.uid),
      getFunnelSends(user.uid), getWhatsappFunnels(user.uid), getDisparos(user.uid),
    ])
    setLeads(ld); setGrupos(gs); setMessageLogs(mlogs); setFunnelSends(sends)
    setWhatsFunnelIds(new Set(wfs.map((f) => f.id))); setDisparos(disp)
    setLoading(false)
  }
  ...
  // Telefones que receberam WhatsApp (remarketing + funil whats + logs de mensagem).
  const telefonesMsg = useMemo(() => {
    const set = new Set()
    for (const l of leads) if (l.status === 'enviado' && l.telefone) set.add(normTel(l.telefone))
    for (const m of messageLogs) { const t = normTel(m.telefone || m.numero); if (t) set.add(t) }
    for (const s of funnelSends) if (whatsFunnelIds.has(s.funnelId)) { const t = normTel(s.contato?.telefone); if (t) set.add(t) }
    return set
  }, [leads, messageLogs, funnelSends, whatsFunnelIds])

  // telefone -> compra {valor,moeda} (mais recente, não estorno) / estorno (soma) / produto / nome
  ...
  const stats = useMemo(() => {
    const disparosCount = disparos.reduce((s, d) => s + (d.enviadosCount ?? d.total ?? 0), 0)
    let enviados
    if (grupoSel) {
      ...
      enviados = telefonesMsg.size + disparosCount
    }
    return { enviados }
  }, [disparos, grupoSel, leads, produtosGrupo, telefonesMsg])
  ...
  // Atividade: compras e estornos dos contatos que receberam WhatsApp
  ...
  const semDados = telefonesMsg.size === 0 && disparos.length === 0

  return (
    <PageShell
      badge="WhatsApp · Métricas"
      ...
      <div className="flex-1 min-w-0 lg:order-2 flex flex-col gap-4 sm:gap-5">
        {semDados && (
          <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-sm">
            Ainda não há dados. Dispare mensagens no WhatsApp (Remarketing, Disparos ou Funil). Quando um contato que recebeu WhatsApp comprar em qualquer webhook, a receita aparece aqui.
          </div>
        )}
        ...
        >
          <div className="overflow-x-auto">
            {atividadeFiltrada.length === 0 ? (
              <p className="p-6 text-sm text-stone-400 text-center">Nenhuma compra/estorno de quem recebeu WhatsApp ainda.</p>
            ) : (
