...
import { getProductGroups, saveProductGroup, deleteProductGroup, getProducts, getLeads, getCheckoutStores } from '../../lib/firestore'
...
export default function EmailProdutos() {
  ...
  const carregar = async () => {
    if (!user?.uid) return
    const [gs, prods, leads, stores] = await Promise.all([getProductGroups(user.uid), getProducts(user.uid), getLeads(user.uid), getCheckoutStores(user.uid)])
    setGrupos(gs)
    const nomes = new Set()
    prods.forEach((p) => p.nome && nomes.add(p.nome))
    leads.forEach((l) => l.produto && nomes.add(l.produto))
    setNomesDisponiveis([...nomes].sort())
    // Achata todos os checkouts (de todas as lojas) pro seletor do contexto.
    setCheckoutsFlat((stores || []).flatMap((s) => (s.produtos || []).filter((p) => p.link).map((p) => ({ nome: p.nome, link: p.link }))))
    setLoading(false)
  }
